...
model decisions): kill-switch → tier → cold gate → recipient guard →
two-phase confirm → send rate cap → alias resolution → handler (on the
EWS pool) → audit.
"""
...
from ..confirm import content_hash, make_token, verify_token
...
def _confirm_gate(ctx: Context, spec: ToolSpec,
    ...
    if not token:
        ...
        external = sorted({
            r for r in _recipients(kwargs) if r.rsplit("@", 1)[-1] != own
        })
    ...
    ok, reason = verify_token(
        ...
        target_id="-", chash=chash, secret=secret,
    )
    if not ok:
        raise ToolError(
            "confirm_invalid",
            f"confirm_token rejected ({reason})",
            hint=f"Call {spec.name} again WITHOUT confirm_token for a fresh "
                 "preview; tokens die when any argument changes or expires.",
        )
    return None


def mint_token(ctx: Context, spec: ToolSpec, kwargs: Dict[str, Any]) -> str:
...
async def dispatch(ctx: Context, spec: ToolSpec, kwargs: Dict[str, Any],
    ...
    try:
        ...
        # Two-phase confirm (hashes the caller's literal args)
        if spec.confirm_needed(kwargs):
            kwargs = dict(kwargs)
            phase1 = _confirm_gate(ctx, spec, kwargs)
        ...
        if spec.side_effect_class in ("send", "destructive"):
            detail = {k: kwargs.get(k) for k in ("to", "cc", "subject", "id", "ids")
